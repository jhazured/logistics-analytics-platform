...
shipments_with_dims as (
  select 
    ...
    dc.customer_tier
  from s
  left join {{ ref('dim_location') }} ol on s.origin_location_id = ol.location_id
  left join {{ ref('dim_location') }} dl on s.destination_location_id = dl.location_id
  left join {{ ref('dim_vehicle') }} dv on s.vehicle_id = dv.vehicle_id

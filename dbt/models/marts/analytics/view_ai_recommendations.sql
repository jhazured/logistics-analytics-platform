...
WITH route_performance_analysis AS (
    ...
    JOIN {{ ref('fact_shipments') }} fs ON dr.route_id = fs.route_id
...
vehicle_utilization_analysis AS (
    ...
    JOIN {{ ref('fact_shipments') }} fs ON dv.vehicle_id = fs.vehicle_id
    WHERE fs.shipment_date >= CURRENT_DATE() - 30
...
customer_insights AS (
    ...
    JOIN {{ ref('fact_shipments') }} fs ON dc.customer_id = fs.customer_id
